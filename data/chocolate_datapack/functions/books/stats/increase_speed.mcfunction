tag @s[scores={skill_points=1..,speed=..99}] add 1
tag @s[scores={skill_points=1..,speed=100..}] add 2
tag @s[scores={skill_points=..1}] add 3

tellraw @s[tag=2] "Your Body Cannot Handle Anymore Speed"
execute at @s[tag=2] run playsound block.fire.extinguish ambient @a ~ ~ ~ 1 2
execute at @s[tag=2] run particle smoke ~ ~1 ~ .5 .5 .5 0 20

tellraw @s[tag=1] "Your speed has increased"
...
tellraw @s[tag=3] "You do not have enough Knowledge Points to increase your speed"

scoreboard players add @s[tag=1] speed 1
