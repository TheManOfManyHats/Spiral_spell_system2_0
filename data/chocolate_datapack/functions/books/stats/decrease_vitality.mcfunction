tag @s[scores={vitality=1..100}] add 1
tag @s[scores={vitality=..0}] add 2

tellraw @s[tag=2] "Your Body Will Fall Apart With Any Less Vitality"
execute at @s[tag=2] run playsound block.fire.extinguish ambient @a ~ ~ ~ 1 2
execute at @s[tag=2] run particle smoke ~ ~1 ~ .5 .5 .5 0 20

tellraw @s[tag=1] "Your vitality has decreased"
...
scoreboard players remove @s[tag=1] vitality 1

attribute @s[tag=1,scores={vitality=0}] generic.max_health base set 20
