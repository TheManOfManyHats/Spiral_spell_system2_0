...
execute at @s[tag=1,scores={spell_power=1}] positioned ~ ~1.5 ~ run summon item_display ~ ~ ~ {Tags:["offensive","self","water","water_pulse","lv1","new_spell","chocolate_datapack_spell"],teleport_duration:1}
#Put kill timer on item_display that lasts for 5 seconds
execute at @s[tag=1,scores={spell_power=1}] positioned ~ ~1.5 ~ run scoreboard players set @e[tag=water_pulse,sort=nearest,limit=1,tag=new_spell] kill_timer 100
#MED POWER
#Summon armor stand that is the actual attack
execute at @s[tag=1,scores={spell_power=3}] positioned ~ ~1.5 ~ run summon item_display ~ ~ ~ {Tags:["offensive","self","water","water_pulse","lv2","new_spell","chocolate_datapack_spell"],teleport_duration:1}
#Put kill timer on item_display that lasts for 5 seconds
execute at @s[tag=1,scores={spell_power=3}] positioned ~ ~1.5 ~ run scoreboard players set @e[tag=water_pulse,sort=nearest,limit=1,tag=new_spell] kill_timer 200
#HIGH POWER
#Summon armor stand that is the actual attack
execute at @s[tag=1,scores={spell_power=5}] positioned ~ ~1.5 ~ run summon item_display ~ ~ ~ {Tags:["offensive","self","water","water_pulse","lv3","new_spell","chocolate_datapack_spell"],teleport_duration:1}
#Put kill timer on item_display that lasts for 5 seconds
execute at @s[tag=1,scores={spell_power=5}] positioned ~ ~1.5 ~ run scoreboard players set @e[tag=water_pulse,sort=nearest,limit=1,tag=new_spell] kill_timer 300
