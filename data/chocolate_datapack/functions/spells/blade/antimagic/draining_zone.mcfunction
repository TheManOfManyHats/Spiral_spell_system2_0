...
#LOW POWER
execute at @s[tag=1,scores={spell_power=1}] positioned ~ ~ ~ run summon item_display ~ ~ ~ {Tags:["offensive","blade","draining_zone","zone","lv1","new_spell","chocolate_datapack_spell","spiral_spell"]}
...
#MED POWER
execute at @s[tag=1,scores={spell_power=3}] positioned ~ ~ ~ run summon item_display ~ ~ ~ {Tags:["offensive","blade","draining_zone","zone","lv2","new_spell","chocolate_datapack_spell","spiral_spell"]}
...
#HIGH POWER
execute at @s[tag=1,scores={spell_power=5}] positioned ~ ~ ~ run summon item_display ~ ~ ~ {Tags:["offensive","blade","draining_zone","zone","lv3","new_spell","chocolate_datapack_spell","spiral_spell"]}
...
execute at @s[tag=1] positioned ~ ~ ~ run scoreboard players operation @e[tag=draining_zone,distance=..1,sort=nearest,tag=new_spell] player_id = @s[tag=1] player_id
#put player's team ID on the spell
execute at @s[tag=1,scores={team_id=1..}] positioned ~ ~ ~ run scoreboard players operation @e[tag=draining_zone,distance=..1,sort=nearest,tag=new_spell] team_id = @s[tag=1] team_id
#Teleport attack item_display to player to match facing when fired
execute at @s[tag=1] rotated as @s positioned ~ ~ ~ run tp @e[tag=draining_zone,sort=nearest,distance=..1,tag=new_spell] ~ ~ ~ ~ 0
#Dramatic flare
execute at @s[tag=1] run playsound minecraft:entity.ghast.shoot ambient @a ~ ~ ~ 1 1
execute at @s[tag=1] run playsound block.anvil.land ambient @a ~ ~ ~ 1 1
##-----------------------------------------------------------------##
...
##-----------------------------------------------------------------##
# Remove tags
execute at @s[tag=1] positioned ~ ~ ~ run tag @e[tag=draining_zone,sort=nearest,distance=..1,tag=new_spell] remove new_spell
tag @s remove 1
