...
#LOW POWER
execute at @s[tag=1,scores={spell_power=1}] positioned ~ ~ ~ run summon item_display ~ ~ ~ {Tags:["offensive","zone","lightning","nimbus","lv1","new_spell","spiral_spell"]}
#MED POWER
execute at @s[tag=1,scores={spell_power=3}] positioned ~ ~ ~ run summon item_display ~ ~ ~ {Tags:["offensive","zone","lightning","nimbus","lv2","new_spell","spiral_spell"]}
#HIGH POWER
execute at @s[tag=1,scores={spell_power=5}] positioned ~ ~ ~ run summon item_display ~ ~ ~ {Tags:["offensive","zone","lightning","nimbus","lv3","new_spell","spiral_spell"]}
...
execute at @s[tag=1] positioned ~ ~1.5 ~ run scoreboard players operation @e[tag=nimbus,sort=nearest,tag=new_spell] player_id = @s[tag=1] player_id
#put player's team ID on the spell
execute at @s[tag=1,scores={team_id=1..}] positioned ~ ~1.5 ~ run scoreboard players operation @e[tag=nimbus,sort=nearest,tag=new_spell] team_id = @s[tag=1] team_id
#Dramatic flare
...
##-----------------------------------------------------------------##
# Remove tag
execute at @s[tag=1] positioned ~ ~1.5 ~ run tag @e[tag=nimbus,sort=nearest,distance=..1,tag=new_spell] remove new_spell
tag @s remove 1