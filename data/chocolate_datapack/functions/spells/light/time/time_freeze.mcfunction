...
execute at @s[tag=1,scores={spell_power=1}] positioned ~ ~1.5 ~ run summon marker ~ ~ ~ {Tags:["offensive","bolt","time","time_freeze","lv1","new_spell","spiral_spell"]}
#MED POWER
execute at @s[tag=1,scores={spell_power=3}] positioned ~ ~1.5 ~ run summon marker ~ ~ ~ {Tags:["offensive","bolt","time","time_freeze","lv2","new_spell","spiral_spell"]}
#HIGH POWER
execute at @s[tag=1,scores={spell_power=5}] positioned ~ ~1.5 ~ run summon marker ~ ~ ~ {Tags:["offensive","bolt","time","time_freeze","lv3","new_spell","spiral_spell"]}
...
execute at @s[tag=1] positioned ~ ~1.5 ~ run scoreboard players operation @e[tag=time_freeze,limit=1,sort=nearest,tag=new_spell] player_id = @s[tag=1] player_id
#put player's team ID on the spell
execute at @s[tag=1,scores={team_id=1..}] positioned ~ ~1.5 ~ run scoreboard players operation @e[tag=time_freeze,limit=1,sort=nearest,tag=new_spell] team_id = @s[tag=1] team_id

#Teleport attack marker to player to match facing when fired
execute at @s[tag=1] rotated as @s positioned ~ ~1.5 ~ run tp @e[tag=time_freeze,sort=nearest,limit=1,tag=new_spell] ~ ~ ~ ~ ~
#Dramatic flare
...
##-----------------------------------------------------------------##
# Remove tag
execute at @s[tag=1] positioned ~ ~1.5 ~ run tag @e[tag=time_freeze,sort=nearest,distance=..1,tag=new_spell] remove new_spell
tag @s remove 1