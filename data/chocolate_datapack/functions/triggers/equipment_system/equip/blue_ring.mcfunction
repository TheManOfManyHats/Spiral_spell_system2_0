...
execute as @s[tag=!blue_ring_equipped,scores={rings_equipped=0..2}] run tag @s add 1
execute as @s[tag=blue_ring_equipped] run tag @s add 2
execute as @s[scores={rings_equipped=3..}] run tag @s add 3

tag @s[tag=1] add blue_ring_equipped
...
tellraw @s[tag=1] {"text":"You have equipped a Blue Ring","color":"gold"}
clear @s[tag=1] stick{Blue_ring:1b,Consumable:1b,CustomModelData:529,display:{Lore:['["",{"text":"A ring with a Blue Pearl","italic":false,"color":"dark_red"}]','["",{"text":"Slotted into it.","italic":false,"color":"dark_red"}]','[""]','["",{"text":"Right click to equip!","italic":false,"color":"gray"}]','["",{"text":"+Strength","italic":false,"color":"dark_green"}]'],Name:'{"extra":[{"bold":false,"italic":false,"underlined":false,"strikethrough":false,"obfuscated":false,"color":"gold","text":"Blue Ring"}],"text":""}'}} 1
