...
execute as @s[tag=!violet_ring_equipped,scores={rings_equipped=0..2}] run tag @s add 1
...
execute at @s[tag=1] run scoreboard players add @s max_wil 10000
...
clear @s[tag=1] stick{Consumable:1b,CustomModelData:531,Violet_ring:1b,display:{Lore:['["",{"text":"A ring with a Violet Pearl","italic":false,"color":"dark_red"}]','["",{"text":"Slotted into it.","italic":false,"color":"dark_red"}]','[""]','["",{"text":"Right click to equip!","italic":false,"color":"gray"}]','["",{"text":"+Max Wil","italic":false,"color":"dark_green"}]'],Name:'{"extra":[{"bold":false,"italic":false,"underlined":false,"strikethrough":false,"obfuscated":false,"color":"gold","text":"Violet Ring"}],"text":""}'}} 1
