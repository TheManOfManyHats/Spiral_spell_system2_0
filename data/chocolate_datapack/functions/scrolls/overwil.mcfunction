...
execute at @s as @e[type=#chocolate_datapack:spell_components,distance=..3,tag=!player_id_giver,tag=!spelljammer,nbt=!{mythicmobs:{}}] unless score @s player_id = @p player_id unless score @s team_id = @p team_id run kill @s